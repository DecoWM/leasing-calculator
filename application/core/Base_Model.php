...
class Base_Model extends CI_Model
{
    ...
    function get_entry($id, $relationships = array())
    {
        if($this->table != null) 
        {
            $id_name = $this->id_name;
            $entry = array_pop($this->db->get_where($this->table, array("$id_name" => $id))->result($this->entity_class));
            if(method_exists($this, 'add_relationships')) $this->add_relationships($entry, $relationships);
            return $entry;
        }
    }

    function list_entries($relationships = array())
    {
        if($this->table != null) 
        {
            $this->db->order_by('last_modified', 'desc');
            $entries = $this->db->get($this->table)->result($this->entity_class);
            if(method_exists($this, 'add_relationships')) $this->add_relationships($entries, $relationships);
            return $entries;
        }
    }

    function list_entries_by_weight($relationships = array())
    {
